function [alpha_deg,delta_deg,lon_wrapped,lat] = groundTrack_J2(input_type,orbit_initial_state,thetaG_0,time_span,omega_E,mu,t0,J2,R_e)
% 
% Function to plot the Ground Track of an orbit during a given time period, taking into account the J2 perturbating effect.
% 
% PROTOTYPE:
...
switch input_type %analize the type of input, and eventually converts it into a Cartesian initial state
    ...
    case 'Cartesian'
        initial_state = orbit_initial_state; %[r;v]
    otherwise
...
[~ , x] = ode113(@(t,x) kepl_orbit_J2(x,mu,J2,R_e),time_span,initial_state,options); %propagates the orbin during the time-span taking into account J2 effect
...
delta_deg = rad2deg(delta); %converts the declination vector in degrees

alpha = NaN(length(time_span),1); %initalization of alpha vector
for i=1:length(time_span) %computes the right ascension
    if (x(i,2)/norm(x(i,1:3)))>0
        alpha(i) = acos((x(i,1)/norm(x(i,1:3)))/cos(delta(i)));
    else
        alpha(i) = (2*pi) - acos((x(i,1)/norm(x(i,1:3)))/cos(delta(i)));
    end
end
alpha_deg = rad2deg(alpha); %converts the right ascension vector in degrees

lat = delta_deg; %computes the latitude vector in degrees

thetaG = ones(length(time_span),1)*thetaG_0 +( deg2rad(omega_E/3600)) * (time_span' - t0*ones(length(time_span),1)); %computes the right ascension of greenwich meridian at each time-step
...
lon_wrapped = wrapTo180(lon); %wraps the longitude between [-180 180]
